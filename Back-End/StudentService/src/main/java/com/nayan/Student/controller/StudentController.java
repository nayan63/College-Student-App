...
public class StudentController {
	...
	@GetMapping("/{id}")
	StudentWithCollege getByIdWithCollege(@PathVariable long id) throws Exception
	{
		try {
			return service.getByIdWithCollege(id);
		} catch (Exception e) {
	...
	String editById(
			...
			@RequestPart("student") Student stu, 
			@RequestPart(name="file", required = false) MultipartFile file) throws Exception
	{
		Student student = service.edit(id, stu,file);
		return student.getName()+" details updated successfully";
	}
